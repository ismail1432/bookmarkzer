...
install: build up vendor assets-build db-load-fixtures ## Up the project and load database

reset: down install ## Reset the project
...
db-load-fixtures: wait-db ## Load database from dump
	...
	@$(EXEC_DB) "mysql --user=root --password=root < /home/app/dump/bookmarkland.sql"
...
db-reload-fixtures: wait-db db-reload-schema ## Reload fixtures
	...
	@echo "\nCreating dump...\e[0m"
	@$(EXEC_DB) "mysqldump --user=root --password=root --databases bookmarkzer_application > /home/app/dump/bookmarkland.sql"

######################################
############ Test ####################
######################################

unit-test: ## Launch unit test
